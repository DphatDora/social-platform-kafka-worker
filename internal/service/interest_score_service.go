...
func (s *InterestScoreService) ProcessInterestScoreUpdate(payloadData json.RawMessage) error {
	...
	if err := json.Unmarshal(payloadData, &p); err != nil {
		logger.Errorf("[Error] Failed to unmarshal interest score payload: %v", err)
		return fmt.Errorf("failed to unmarshal payload: %w", err)
	}

	// Validate payload
	if p.UserID == 0 || p.CommunityID == 0 {
		logger.Warnf("[Warning] Invalid payload: UserID=%d, CommunityID=%d", p.UserID, p.CommunityID)
	...
	if err := s.interestScoreRepo.CreateOrUpdate(p.UserID, p.CommunityID, scoreDelta, p.Action); err != nil {
		logger.Errorf("[Error] Failed to update interest score: %v", err)
		return fmt.Errorf("failed to update interest score: %w", err)
	}

	logger.Infof("[InterestScore] Updated successfully for UserID=%d, CommunityID=%d", p.UserID, p.CommunityID)
	return nil
}
...
func (s *InterestScoreService) GetScoreDeltaForAction(action string) float64 {
	...
	default:
		logger.Warnf("[Warning] Unknown action: %s, returning 0", action)
		return 0.0
	}
}
