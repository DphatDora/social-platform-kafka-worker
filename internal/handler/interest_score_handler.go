...
func (h *InterestScoreHandler) ProcessInterestScoreBotTasks(ctx context.Context, batchSize int) error {
	...
	if err != nil {
		logger.Errorf("[Error] Failed to fetch interest score bot tasks: %v", err)
		return err
	}
	...
	logger.Infof("[InterestScore] Found %d bot tasks to process", len(tasks))
	...
	for _, task := range tasks {
		if err := h.ProcessSingleTask(&task); err != nil {
			logger.Errorf("[Error] Failed to process task ID=%d: %v", task.ID, err)
			errorCount++
			continue
		}
		...
		if err := h.taskRepo.MarkAsExecuted(task.ID, now); err != nil {
			logger.Errorf("[Error] Failed to mark task ID=%d as executed: %v", task.ID, err)
			errorCount++
			continue
		}
	...
	logger.Infof("[InterestScore] Processed %d tasks: %d successful, %d errors", len(tasks), successCount, errorCount)
	return nil
}
